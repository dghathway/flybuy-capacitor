...
public class CapFlyBuyPlugin: CAPPlugin {
    ...
    @objc func createOrder(_ call: CAPPluginCall) {
        guard let order = call.getArray("order")?[0] as? NSDictionary else {
            return call.resolve([
                "status": "error",
                "messageAs": "Incorrect parameter order"
            ])
        }
        let siteId = order["siteId"] as? Int
        let partnerId = order["partnerId"] as? String
        let customerInfo = order["customer"] as? NSDictionary
        let customer = CustomerInfo(fromInfo: customerInfo ?? [:])
        
        var pickupWindow: PickupWindow?
        if let pickupWindowArg = order["pickupWindow"] as? NSDictionary,
        ...
        FlyBuy.Core.orders.create(siteID: siteId ?? 0, partnerIdentifier: partnerId ?? "0", customerInfo: customer, pickupWindow: pickupWindow) { (order, error) in
            if let error = error {
