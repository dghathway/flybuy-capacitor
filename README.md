...
```typescript
createCustomer(options: { customer: Customer[]; }) => any
```
...
```typescript
updateCustomer(options: { customer: Customer[]; }) => any
```
...
```typescript
createOrder(options: { order: Order[]; }) => any
```
...
```typescript
requestPermissions(options: { permissions: boolean[]; }) => any
```
